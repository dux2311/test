/* --|-------------------------------------------------------|
 * --|-----------.Script Written by Nineza.------------------|
 * - |-----------.Modified by Ibra Gonza II------------------|
 * - |------- Nineza's Awesome Report Renamer!---------------|
 * - |-------------------------------------------------------|
 * - |-------------------------------------------------------|
 * - -> Bugs? What are bugs?
 *
...
{Farm} - Number of LC needed to empty the city of all resources (e.g 107)
...
function rename(format) {
    ...
    var LCCarry = 80;
    var wood, clay, iron, wall, farm, sum;
    if (espionage.length > 0) {
        ...
        sum = parseInt(wood) + parseInt(clay) + parseInt(iron);
        farm = Math.ceil(parseInt(sum) / parseInt(LCCarry));
    ...
    if (defenderTroopsRemainingString === "") var cleared = _("Cleared");
    else var cleared = "";
    ...
    setTimeout(function () {
        $(".quickedit input[type='text']").val(name);
        setTimeout(function () {
            $(".quickedit input[type='button']").click();
        }, 250);
    }, 100);
}
